...
/app
  ...
  /api/cron/process-staged/route.ts    → batches + judges staged_context_entries (added 2026-09-12)
...
**`staged_context_entries`** (added 2026-09-12) — `id`, `user_id`, `raw_text`, `kind_hint`, `scope_hint`, `source`, `created_at`. Write-only landing zone for the `remember` MCP tool (§6); drained and deleted by `app/api/cron/process-staged`. Never read by the app UI.
...
`remember` never calls an LLM on the request path — it does cheap dedupe (`stageEntry()` in `lib/db.ts`) and writes to `staged_context_entries`, returning immediately. A separate batched job (`app/api/cron/process-staged/route.ts`, Vercel cron every 5 minutes — **verify this cadence actually runs on whatever Vercel plan is in use; some plans limit cron frequency**) judges up to 20 staged rows in a single LLM call, writes the kept ones into `context_entries` with a `kind`, and deletes all processed rows (kept or dropped) so the staging table never grows unbounded. This exists specifically so no chat response ever waits on an LLM call to save something (see ROADMAP.md P1.4 for the reasoning — the two-stage split is load-bearing, not incidental).
...
- **New 2026-09-12:** the `/api/cron/process-staged` schedule (`*/5 * * * *` in `vercel.json`) assumes frequent cron execution. Verify this actually runs at that cadence on whatever Vercel plan is deployed — some plans limit cron job frequency, which would mean `remember`-staged entries sit unprocessed longer than intended (harmless — nothing is lost, `context_entries` just fills in slower than expected).
- **New 2026-09-12:** if `judgeContext()` fails repeatedly (both Gemini and the OpenRouter cascade down), `process-staged` leaves the whole batch staged for retry rather than dropping it — correct for not losing data, but there's currently no cap on how long a batch can sit retrying, nor alerting if it does.
